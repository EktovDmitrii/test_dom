...
val dataModule = module {
    single <RegistrationRepository> { RegistrationRepositoryImpl(api = get(), clientSharedPreferences = get(), webSocketRepository = get(), context = get()) }
    ...
    single <WebSocketRepository> {WebSocketRepositoryImpl(clientSharedPreferences = get(), gson = get(), context = get())}

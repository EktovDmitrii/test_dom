...
class PromoCodesRepositoryImpl(private val api: MSDApi) : PromoCodesRepository {
    ...
    override fun getPromoCodes(): Single<List<PromoCodesItemModel>> {
        return api.getPromoCodes(statuses = PROMO_CODE_STATUSES).flatMap {
            Single.just(PromoCodesMapper.responseToPromoCodesItemModels(it.items))
        }
    }

    override fun activatePromoCode(promoCodeId: String): Single<PromoCodesItemModel> {
        val bodyRequest = ActivatePromoCodeRequest(promoCode = promoCodeId)
        return api.activatePromoCode(body = bodyRequest).flatMap {
            Single.just(PromoCodesMapper.responseToPromoCodesItemModel(it))
        }
    }
}
